...
This vignette will demonstrate use of `EGprocess` during the escapement goal review process using the Igushik River Sockeye salmon stock as an example. The BOF last considered Bristol Bay salmon stocks during a meeting on January 13-18, 2026. Prior to this meeting, ADF&G staff decided to updated the Igushik River Sockeye salmon analysis and data containing that updated analysis is loaded into `EGprocess`.  
...
`EGprocess` contains helper functions to create the brood table from "Run" type data. The function `make_age` creates the age composition used in the spawner-recruit analysis and is a required input to build the brood table. The function `make_brood` creates the brood table itself. Notice that the results is 'jagged' and includes escapements (`S`), recruits (`R`) and return-by-age (`b.Age#`) which are missing. For example, the Igushik River Sockeye salmon analysis update conducted prior to the 2026 BOF meeting utilized escapement information through 2023 but only had recruitment through 2015 because 8 year old fish from the 2015 escapement were not observed until 2023. 
...
[^3]: Staff knowledgeable about the history of Bristol Bay sockeye salmon escapement goals will recognize this statement is not strictly true. Instead the file, `post_Igushik_byr63_05` contains model output from using only the 1963 - 2005 brood years because we were unable to locate the actual posterior produced in 2015. This is sufficient to demonstrate use of the `EGprocess` package and not intended to provide complete historic fidelity.   

```{r}
head(post_Igushik_byr63_05)
```

In the State of Alaska most salmon stocks have existing escapement goals and those escapement goals represent a sweet spot that accounts for not only th information provided thby the analysis wrt how to maximize sustained yield but also concerns about stock status that are poorly reflected in the analysis and and escapement goal range that effectively persecutes the fishery. For these reasons, it is often helpful to consider how the updated escapement goal analysis has changed our interpretation of the existing goal before considering if a new finding is appropriate. `EGprocess` can help make this comparison if both sets of posterior data are provided to the functions as a list with the posteriors names and provided in chronological order.
...
Some `EGprocess` functions utilize the function argument `profile_data` which is a derivative of the `posterior data` files described above and is required to build optimal yield profiles (OYP) and expected yield plots. A helper function is provided to create `profile_data` from `posterior_data`. 
...
The simplest way to retrieve the output from `EGprocess` is to run the function `EGoutput` which is a wrapper that calls all of the outputs within the package. Some commonalities you will find in all figures produced by this package:
...
-   Use of Arial font.
-   Notes associated with each figure which provide a terse description of the figure's purpose and a textual legend. This approach maximized the percentage of the figure dedicated to displaying the data, ensures the legend travels with the figure, and allows for brief report captions.
-   Users retain the ability to customize figures to stock specific situations.
```{r}
# Need to correct this function name to make file name match.
...
EGPIT decided historical escapements should be depicted as a bar plot overlain with the contemporary escapement goal range and color coded to indicate escapements below the lower bound of the contemporary escapement goal.

<br>

![Figure 1: Igushik River sockeye salmon escapement, 1963-2025.](../man/figures/Igushik_escapement.png)
...
EGPIT decided escapement goals intended to target MSY should use a universal standard target yield of greater than 90% of MSY. When producing OYP plots this means a single line, overlain with the current escapement goal is needed to describe the probability of maximizing yield within the escapement goal range.
...
During the escapement goal review process for Igushik River Sockeye salmon the escapement goal review committee elected not to change the escapement goal. In the event the committee felt an new escapement goal finding was appropriate the `EGprocess` outputs can reflect that change by modifying the `goal_dat` file. 
...
The new escapement goal finding is overlain on the optimal yield profile for the updated analysis while the current escapement goal range is overlain on the optimal yield profile concurrent with that escapement goal change. This configuration allows staff to compare the degree of similarity between their new and past escapement goal findings with respect to maximizing MSY.
...
In what follows we will also change the figure titles to make clear that this is an imaginary example. Notice that `profile_data` is provided a single profile file since there is not prior analysis to compare too.
```{r}
# Need to correct this function name to make file name match.
# Should name the elements in the output list
output_nonexistant <-
...
First time escapement goal findings are overlain on both the spawner-recruit and expected yield (not shown) figures.
...
EGPIT decided escapement goals intended to target MSY should use a universal standard target yield of greater than 90% of MSY. A standard target of 90% of MSY makes sense for most salmon fisheries in Alaska. Arguably, Igushik River sockeye salmon is not one of them. The first issue is practical, since the lower bound of the escapement goal offers near 0% probability of achieving 90% of MSY the OYP in Figure 4 does a poor job of discriminating between potential changes to the lower bound of the escapement goal. Taken alone, that might be ok. Figure 4 demonstrates the the current goal is sub optimal with respect to a target of 90% of MSY and makes it clear that even lower escapement goal lower bounds would be even less optimal. However, Bristol Bay stakeholders have expressed a preference for sub optimal yield to ensure that fisheries open regularly and the fishery harvests do not exceed processor capacity[^4]. In situations like this, where staff and stakeholders can clearly describe a need and/or preference to target sub optimal yield, profile plots can be modified to evaluate escapement goals relative to both optimal and sub optimal yield. This can be achieved by specifying the sub optimal yield level to the function `get_profile`.
...
![Figure 9: Igushik river sockeye salmon optimal yield profile.](../man/figures/Igushik_profile_80.png)
